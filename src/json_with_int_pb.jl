...
function meta(::Type{Value})
    ProtoBuf.metalock() do
        if !isassigned(__meta_Value)
            ...
            allflds = Pair{Symbol,Union{Type,String}}[:null_value => Int32, :double_value => Float64, :integer_value => Int64, :string_value => AbstractString, :bool_value => Bool, :struct_value => Struct, :list_value => "ListValue"]
